        ...
        <div class="col-12">
            ...
            <div class="row bg-blue">
                <div class="form-group col-4">
                    <h3 class="head">ALL AVAILABLE TRAINS</h3> <hr> 
                </div>
                <div class="col-md-8">
                {{ Form::open(['route' => 'trains.store', 'class' => 'form-inline justify-content-end'])}}
                    <div class="form-group">
                        {{ Form::text('name', null, ['class' => 'form-control form-control-sm', 'placeholder' => 'Add a new train'])}}
                        {{ Form::text('number', null, ['class' => 'form-control form-control-sm', 'placeholder' => 'Add a train"s number'])}}
                    </div>
                                ...
                                <tbody>
                                    @foreach ($trains as $train)
                                        <tr>
                                            ...
                                            <td>{{ $train->name}}</td>
                                            <td>
                                                <a href="{{ route('customers.create', $train->id) }}" class="btn btn-secondary btn-sm editBtn" data-id="{{$train->id}}">Book</a>
                                            </td>
                                            <td>
                                                <a href="{{ route('trains.show', $train->id) }}" class="btn btn-info btn-sm">View</a>
                                            </td>
                                            <td>
                                                {{-- <a href="" class="btn btn-danger btn-sm {{ count($train->members) > 0 ? 'disabled' : ''}} removeBtn" data-id="{{$train->id}}" data-name="{{$train->name}}">Remove</a> --}}
                                            </td>
                                        </tr>                               
                                    @endforeach
                                </tbody>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">Edit train</h5>
            ...
            <div class="modal-body">
                ...
                {{-- {{ Form::open(['route' => 'trains.update', 'id' => 'form-update'])}} --}}
                    <div class="form-group">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="text-danger fs-14">
                        ...
                        Removed trains CANNOT be restored! Are you ABSOLUTELY sure?
                    </div>
                    <div class="text-monospace fs-12 m-t-20">
                        This action will delete all members associated with this train
                    </div>
...
@push('scripts')
    ...
    * Makes an ajax call to get the train model belonging to that id
    ...
    */
        $('.editBtn').click(function(e) {
            ...
            $('.alert-danger').hide();
            var id = $(this).data('id');
            $.get("{{route('trains.index')}}", {id:id}, function(data){
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.errors) {
                        ...
                        window.train.href = "{{route('trains.index')}}";  
                    }
                },
                error: function() {
    ...
    */
        $('#modalConfirmBtn').click(function(e) {
            ...
            $.get("{{route('trains.index')}}", {id:id}, function(){
                window.train.href = "{{route('trains.index')}}"; 
            })
        })
    </script>
